...
export default function ExclusiveTips() {
  ...
  const unlockedCount = exclusiveTips.filter(tip => canAccess("exclusive")).length;
  ...
  return <div className="section-gap">
      ...
      <div className="flex items-center justify-between gap-1.5 p-3 rounded-lg bg-gradient-to-r from-violet-500/20 via-purple-500/10 to-transparent border border-violet-500/30 shadow-[0_0_15px_rgba(139,92,246,0.15)]">
        <div className="flex items-center gap-1.5">
          ...
          <div>
            <h1 className="text-xs text-foreground font-semibold sm:text-lg">Pro Tips</h1>
            <p className="text-[8px] sm:text-[9px] text-muted-foreground">Premium predictions with higher confidence</p>
          </div>
        </div>
        <div className="flex items-center gap-1">
          <Badge variant="outline" className="bg-violet-500/20 text-violet-400 border-violet-500/30 text-[9px] px-1.5 py-0.5">
      ...
      {showUpgradeBanner && <div className="p-2 sm:p-3 rounded-lg bg-gradient-to-r from-violet-500/20 via-purple-500/10 to-transparent border border-violet-500/30 shadow-[0_0_15px_rgba(139,92,246,0.15)]">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="font-semibold text-[10px] sm:text-xs text-violet-300">Unlock All Pro Tips</h3>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        return <TipCard key={tip.id} tip={{
          ...
          tier: tip.tier
        }} isLocked={isLocked} unlockMethod={unlockMethod} onUnlockClick={() => handleUnlock("tip", tip.id, "exclusive")} />;
      })}
      </div>
    </div>;
}
