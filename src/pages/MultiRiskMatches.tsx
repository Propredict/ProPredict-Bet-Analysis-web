...
import { Target, RefreshCw, BarChart3, TrendingUp, Loader2, Ticket, Crown } from "lucide-react";
...
export default function MultiRiskMatches() {
  ...
  const handleRefresh = async () => {
    ...
    try {
      await refetch();
      toast({ title: "Data refreshed", description: "Multi Risk Matches have been updated." });
    } finally {
        ...
        <div className="flex items-center justify-between gap-1.5 p-3 rounded-lg bg-gradient-to-r from-orange-500/20 via-red-500/10 to-transparent border border-orange-500/30 shadow-[0_0_15px_rgba(249,115,22,0.15)]">
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <div>
              ...
              <p className="text-[9px] sm:text-[10px] text-muted-foreground">High-risk multi-match combos</p>
            </div>
          </div>
          <div className="flex items-center gap-1 sm:gap-1.5">
            <Badge className="bg-orange-500/20 text-orange-400 border-orange-500/30 text-[9px] sm:text-[10px] px-1.5 py-0.5">
              🔥 Risky
            </Badge>
            <Button variant="outline" size="sm" onClick={handleRefresh} disabled={isRefreshing || isLoading} className="gap-0.5 h-6 sm:h-7 px-1.5">
          ...
          {isLoading ? (
            ...
            <Card className="p-8 bg-card border-border">
              <div className="flex flex-col items-center justify-center text-muted-foreground">
                ...
                <p className="text-warning mb-1">No risk combos available</p>
            ...
            premiumTickets.map((ticket, idx) => {
              ...
              return (
                <React.Fragment key={ticket.id}>
                  ...
                  />
                  {(idx + 1) % 5 === 0 && idx < premiumTickets.length - 1 && <AdSlot className="col-span-full" />}
                </React.Fragment>
              );
            })
          )}
