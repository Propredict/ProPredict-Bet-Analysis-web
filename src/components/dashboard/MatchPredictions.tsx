...
export function MatchPredictions() {
  ...
  return <section className="space-y-1.5 sm:space-y-2">
      <div className="flex items-center gap-1.5">
        ...
        <h2 className="text-xs font-semibold sm:text-lg">Daily Tips </h2>
      </div>

      <Card className="p-0.5">
        <div className="grid grid-cols-3 gap-0.5">
          {tabs.map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)} className={cn("py-1.5 sm:py-2 rounded-md text-[10px] sm:text-xs", activeTab === tab.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted")}>
      ...
      {isLoading ? <div className="flex justify-center py-6">
        ...
        </div> : filteredTips.length > 0 ? <div className="space-y-1.5 sm:space-y-2">
          {filteredTips.map(tip => {
        const isLocked = !canAccess(tip.tier, "tip", tip.id);
        const unlockMethod = getUnlockMethod(tip.tier, "tip", tip.id);
        return <TipCard key={tip.id} tip={tip} isLocked={isLocked} unlockMethod={unlockMethod} isUnlocking={unlockingId === tip.id} onUnlockClick={() => handleUnlock("tip", tip.id, tip.tier)} />;
      })}
