...
export function BettingTickets() {
  ...
  const tabs = [{
    ...
    label: "Daily",
    icon: Sparkles,
    sublabel: "Free with Ads"
  }, {
    id: "exclusive" as TabType,
    label: "Pro",
    icon: Star,
    sublabel: "Pro Members"
  }, {
  ...
  return <section className="space-y-1.5 sm:space-y-2">
      ...
      <div className="flex items-center gap-1.5">
        <Ticket className="text-primary w-[20px] h-[20px]" />
        <h2 className="text-xs font-semibold text-foreground sm:text-lg">Daily Tickets</h2>
      </div>
      ...
      <Card className="p-0.5 bg-card border-border">
        <div className="grid grid-cols-3 gap-0.5">
          ...
          return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn("flex flex-col items-center gap-0.5 py-1.5 sm:py-2 px-1.5 sm:px-2 rounded-md transition-all", activeTab === tab.id ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-muted")}>
                <div className="flex items-center gap-1">
                  <tab.icon className="h-3 w-3" />
                  <span className="font-medium text-[10px] sm:text-xs">{tab.label}</span>
                  <span className={cn("text-[9px] px-1 py-0.5 rounded-full", activeTab === tab.id ? "bg-primary-foreground/20" : "bg-muted")}>
                    {count}
                  </span>
                </div>
      ...
      {isLoading ? <div className="flex justify-center py-6">
        ...
        </div> : filteredTickets.length > 0 ? <div className="grid md:grid-cols-2 gap-1.5 sm:gap-2">
        ...
        return <TicketCard key={ticket.id} ticket={ticket} isLocked={isLocked} unlockMethod={unlockMethod} onUnlockClick={() => handleUnlock("ticket", ticket.id, ticket.tier)} onViewTicket={() => navigate(`/tickets/${ticket.id}`)} isUnlocking={isUnlocking} />;
      })}
        </div> : <Card className="p-4 text-center">
