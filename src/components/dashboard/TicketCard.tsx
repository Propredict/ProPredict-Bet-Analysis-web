...
function getLockedCTAText(unlockMethod: UnlockMethod): string {
  ...
  if (unlockMethod.type === "upgrade_premium") return "Get Premium / Kupi Premium";
...
function TicketCard({
  ...
  const renderHeader = () => (
    <>
      <div className={cn("h-1.5 w-full", light ? "bg-gradient-to-r from-sidebar via-primary to-primary" : accent.line)} />
      <div className={light ? "bg-gradient-to-b from-secondary to-card border-b border-border" : cn("bg-gradient-to-b", accent.gradient)}>
          ...
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-1.5">
              ...
              <span className={cn(
                ...
                light ? "text-primary bg-primary/10 border-primary/20" : "text-muted-foreground bg-muted/40 border-border/30"
              )}>
          ...
          <div className={cn(
            ...
            {ticket.totalOdds > 0 && (
              <span className="flex items-center gap-1">
                ...
                <span className={cn("font-bold", light ? "text-primary" : "text-foreground")}>{formatCombinedOdds(ticket.totalOdds)}</span>
              </span>
            )}
          </div>
        </div>
              ...
              return (
               <div key={idx} className="rounded-lg border-2 border-primary/50 bg-secondary/45 p-2.5">
                  ...
                  <div className="mt-1.5 flex items-center justify-center gap-2">
                    <span className={cn("text-[10px] flex items-center gap-1 shrink-0", "text-muted-foreground")}>
      ...
      <div className="space-y-1.5 px-2 pb-2 pt-2 sm:space-y-2 sm:px-4">
        ...
        {displayedMatches.length > 0 ? (
          displayedMatches.map((match, idx) => {
            const parsed = parseMatchName(match.name);
            return (
              <div key={idx} className={cn("grid min-w-0 grid-cols-[2rem_minmax(0,1fr)_3.5rem] items-center gap-2 rounded-lg border border-primary/20 bg-card p-2.5", light && "border-primary/25")}>
                <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary text-xs font-extrabold text-primary-foreground">{idx + 1}</div>
                {parsed.league && (
