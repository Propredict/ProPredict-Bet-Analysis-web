...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`[send-win-push] tier=${record.tier}, route=${route}, eligible=${eligiblePlayerIds.length}/${tokensByUser.size}`);
    ...
    return new Response(JSON.stringify({ success: true, onesignal: result, targets: eligiblePlayerIds.length }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error: unknown) {
