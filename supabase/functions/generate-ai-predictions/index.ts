...
// HARD FLOOR: anything below 55% is HIDDEN. We refuse to publish predictions
// for matches where API-Football lacks sufficient historical data (form,
// season stats, h2h) — those rows are deleted upstream in the data quality gate.
...
async function processBatch(
  ...
  for (const pred of predictions) {
    try {
      ...
      // Non-WC matches are generated normally (no stagger).
      try {
        if (leagueId === 1) {
      ...
      // Freeze WC picks once they are enriched — World Cup predictions become
      // final ~3h before kickoff and must NEVER change after users have seen them.
